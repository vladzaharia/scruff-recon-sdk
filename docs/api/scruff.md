...
| Field | Value | Notes |
|---|---|---|
| `device_id` | `droid-<40 hex>` | **The entire credential.** See [§3](#3-authentication). |
| `hardware_id` | `droid-<hex>` | Stable per install. Suppressed if it would be `droid-null` or `droid-000000000000000`. |
...
**The server does not rate-limit** — across three captures there is not a single 429, nor
any `X-RateLimit-*` or `Retry-After` header.
...
⚠️ **`429` here is SCRUFF's own "rate limit" code, not a standard one**, and it arrives
with no `Retry-After`. See [§1.4](#14-rate-limiting) — no 429 was ever produced during
capture, so the trigger threshold is unknown.
...
| Id | Shape | Role |
|---|---|---|
| `device_id` | `droid-<40 hex>` | **The session credential.** Client-generated, bound to the account by `connect`. |
...
| `device_id` | **client-generated**, `droid-<40 hex>` |
...
1. Generate and persist `hardware_id` (UUID v4), `device_id` (`droid-` + 40 hex),
