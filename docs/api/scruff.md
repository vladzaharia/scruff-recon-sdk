...
| Field | Value | Notes |
|---|---|---|
| `device_id` | `droid-<32 hex>` (38 chars) | **The entire credential.** See [§3](#3-authentication). |
| `hardware_id` | `droid-<16 hex>` (22 chars) | Stable per install, derived from the Android id. **Not a UUID.** Suppressed entirely if it would be `droid-null` or `droid-000000000000000`. |
...
| Id | Shape | Role |
|---|---|---|
| `device_id` | `droid-<32 hex>` | **The session credential.** Client-generated, bound to the account by `connect`. |
...
- `device_id` is **generated by the client**, not issued by the server. It is
  `"droid-" + 32 hex chars`, i.e. 38 characters in total.
...
| `device_id` | **client-generated**, `droid-<32 hex>` |
...
1. Generate and persist `hardware_id` (`droid-` + 16 hex), `device_id` (`droid-` + 32 hex),
